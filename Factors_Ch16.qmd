...
## Comparing Number, Character and Factor Types {.smaller}

| Characteristic         | Numeric                                   | Character                                  | Factor                                     |
|------------------------|-------------------------------------------|--------------------------------------------|--------------------------------------------|
| Representation of Data | Numbers                                   | Text                                       | Categories/Levels                          |
| Internal Representation| Stored as numbers                         | Stored as text strings                     | Stored as integers and labels             |
| Operations             | Mathematical operations                   | Concatenation (e.g., `paste()`)            | Statistical analysis and plotting         |
| Level of Measurement   | Interval or ratio                         | Nominal                                    | Nominal or ordinal depending on context   |
| Handling Missing Values| `NA`                                      | `""` or `NA`                               | `NA` but sometimes special categories                                      |
...
::: aside
**Source**: Kaggle [https://www.kaggle.com/kaggle/kaggle-survey-2017](https://www.kaggle.com/kaggle/kaggle-survey-2017)
:::
...
## Clean Up and Shorten a Few Labels {.smaller}

* New labels are on the left

* The name for China has an extra space and an apostrophe 

* Put in common abbreviations for UK and USA

* Recode "Republic of China" as "Taiwan".
...
## Lump smaller countries together to focus on the largest {.smaller}

Use `fct_recode()` to combine new "All Other" with previous "Other"

```{r}
ks_data2 <- mutate(ks_data2, top_country = fct_lump_prop(Country, prop = .01, 
                                                         other_level = "All Other")) |> 
    mutate(top_country = fct_recode(top_country, "Other" = "All Other"))
...
ks_data2 |>  group_by(top_country) |> 
  ...
  ggplot(aes(x = mean_age, y = top_country)) + 
  geom_point()
```
