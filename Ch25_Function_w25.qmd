...
# Functions and Automation: Scotland First Minister's COVID speeches  {background-color="#FAD9C7"}
...
![](figs/fm_speeches_picture_oct20.png){.absolute top=300 right=100  height="150"}
...
Run this code to see the long output:
...
# Functions to the Rescue  {background-color="#FAD9C7"}
...
## How many times would we need to copy and paste the code we developed to scrape data on all of First Minister's COVID-19 speeches? {.smaller}
...
### Assuming that the page structure is the same for each speech page, how many "things" do you need to know for each speech page to scrape the data we want from it? {.smaller}
...
## Define the function scrape_speech()  {.smaller}
...
## Function scrape_speech() in action {.smaller}
...
# Inputs  {background-color="#FAD9C7"}
...
# Iteration  {background-color="#FAD9C7"}
...
## Learn about the package `purrr` {.slide background-image="figures/purrr.png"}
