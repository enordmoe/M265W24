...
* Use simple filters
* Use `is.na()` examples (and as part of other conditions say, before March and after March)
...
Find all the flights that left from O'Hare (ORD) before noon destined for Orlando (MCO) and arrived more than one hour late.
...
Find all flights to Florida that were either canceled or left on the hour
...
Check if `any()` or `all()` conditions are true for the `group_by()` group:
    + During which hour spans were any or all flights out of O'Hare?
...
Find all the days where more than half of the non-cancelled flights were delayed in departing:
...
Find all the dates on which more than half of the flights were either cancelled or delayed in departing
...
Create a new `timeofday` variable
    * Value assigned based on first TRUE condition
